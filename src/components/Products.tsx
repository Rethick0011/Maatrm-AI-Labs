const products = [
  { name: 'Operational Solution Tailored', desc: 'Custom systems for billing, inventory, product tracking, customer records, employee coordination, approvals, reporting, and multi-user collaboration.' },
...
export default function Products() {
  return (
    <section id="solution-areas" className="py-24 bg-bg-base">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h2 className="font-heading text-4xl font-bold text-white text-center mb-6 tracking-tighter">
          Solution Tailored to your business model
        </h2>
        ...
        <div className="grid items-stretch md:grid-cols-3 gap-8">
          {products.map((p) => (
            <div key={p.name} className="flex h-full min-h-[260px] flex-col rounded-2xl border border-border bg-bg-card p-8 transition-all hover:border-gold hover:scale-[1.02]">
              <div className="mb-6 h-1.5 w-16 rounded-full bg-gradient-to-r from-gold-soft to-gold" />
              <h3 className="font-heading text-2xl font-bold text-gold mb-4 leading-tight">{p.name}</h3>
